...
def _replace_path_root(value, source_root: Path, destination_root: Path):
    """Replace staging paths in a report with their committed artifact paths."""
    if isinstance(value, dict):
        return {
            key: _replace_path_root(item, source_root, destination_root)
            for key, item in value.items()
        }
    if isinstance(value, list):
        return [_replace_path_root(item, source_root, destination_root) for item in value]
...
class Exporter:
    ...
    def __init__(
        ...
        *,
        ...
        self.public_output_dir = (
            Path(public_output_dir) if public_output_dir is not None else self.output_dir
        )
        self.in_dim = in_dim
    ...
    def run(self, skip_validation: bool = False) -> dict:
        ...
        report = {
            ...
            "output_dir": str(self.public_output_dir),
            "in_dim": self.in_dim,
        ...
        if not skip_validation:
            report["validation"]["portability"] = self._safe_validation(
                "Portability validation",
                lambda: self._validate_portability(model),
            )
        ...
        if "onnx" in self.formats:
            ...
            if not skip_validation and onnx_report.get("status") == "success":
                report["validation"]["onnx_numerical"] = self._safe_validation(
                    "ONNX numerical validation",
                    lambda: self._validate_onnx(model, onnx_path),
                )
        ...
        if "torchscript" in self.formats:
            ...
            if not skip_validation and ts_report.get("status") == "success":
                report["validation"]["torchscript_numerical"] = self._safe_validation(
                    "TorchScript numerical validation",
                    lambda: self._validate_torchscript(model, ts_path),
                )
            elif not skip_validation:
                report["validation"]["torchscript_numerical"] = {
                    "status": "skipped",
                    "reason": "TorchScript export failed",
                }

        # 5. Copy checkpoint as fallback
        fallback_path = self.output_dir / "model.ckpt"
        fallback_tmp = fallback_path.with_suffix(".ckpt.tmp")
        try:
            shutil.copy2(self.checkpoint_path, fallback_tmp)
            fallback_tmp.replace(fallback_path)
        finally:
            fallback_tmp.unlink(missing_ok=True)
        report["exports"]["checkpoint"] = {
            "path": str(fallback_path),
            "size_mb": round(fallback_path.stat().st_size / (1024 * 1024), 2),
            "status": "success",
        }

        report["elapsed_seconds"] = round(time.monotonic() - start, 1)
        report["success"] = _all_validations_passed(report)
        report = _replace_path_root(report, self.output_dir, self.public_output_dir)
    ...
    @staticmethod
    def _safe_validation(name: str, validation: Callable[[], dict]) -> dict:
        """Turn validation runtime errors into a fail-closed report entry."""
        try:
            return validation()
...
def _all_validations_passed(report: dict) -> bool:
    """Check that every requested export and validation step succeeded."""
    exports = report.get("exports", {})
    requested_formats = report.get("formats", [])
    for name in requested_formats:
        result = exports.get(name, {})
        if not isinstance(result, dict) or result.get("status") != "success":
            return False
    for _name, result in exports.items():
