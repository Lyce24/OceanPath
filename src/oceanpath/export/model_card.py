...
class ModelCard:
    ...
    def add_metrics(self, metrics: dict[str, Any]) -> "ModelCard":
        """Add training/validation metrics (e.g., from OOF evaluation)."""
        self._metrics.update(metrics)
        return self

    def add_thresholds(self, thresholds: dict[str, Any]) -> "ModelCard":
    ...
    def write(self, path: str | Path) -> Path:
        ...
        card = self.build()
        output_path.write_text(json.dumps(card, indent=2, default=str))
...
def load_oof_metrics(train_dir: str | Path) -> dict[str, Any]:
    """
    ...
    Falls back to: {train_dir}/eval/evaluation_report.json (from Stage 5).
    """
    train_path = Path(train_dir)

    # Try OOF metrics (from training)
    oof_path = train_path / "oof_metrics.json"
    if oof_path.is_file():
        try:
            payload = json.loads(oof_path.read_text())
            if isinstance(payload, dict):
                return payload
            logger.warning(f"Expected a JSON object in {oof_path}")
        except Exception as e:
            logger.warning(f"Cannot parse {oof_path}: {e}")

    # Fallback: evaluation report from Stage 5
    eval_path = train_path / "eval" / "evaluation_report.json"
    if eval_path.is_file():
        try:
            full_report = json.loads(eval_path.read_text())
            if isinstance(full_report, dict):
                metrics = full_report.get("metrics", full_report)
                if isinstance(metrics, dict):
                    return metrics
            logger.warning(f"Expected metrics JSON object in {eval_path}")
        except Exception as e:
...
def load_threshold_info(eval_dir: str | Path) -> dict[str, Any]:
    """
    Load threshold calibration from Stage 5.
    ...
    if path.is_file():
        try:
            ...
            if isinstance(payload, dict):
                return payload
            logger.warning(f"Expected a JSON object in {path}")
        except Exception as e:
...
def _nested_get(d: dict[str, Any], dotted_key: str, default: Any = None) -> Any:
