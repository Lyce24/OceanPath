...
# ── Slide ID normalization (shared with linear_probing_sklearn.py) ────────────
...
def _normalize_slide_id(x: str) -> str:
    ...
    - Does NOT strip arbitrary suffixes (e.g. TCGA UUIDs)
    """
...
class MILDataModule(L.LightningDataModule):
    ...
    def _load_labels(self) -> dict[str, int]:
        ...
        df["slide_id"] = df[self.filename_column].astype(str).map(_normalize_slide_id)
