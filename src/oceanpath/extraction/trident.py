...
def compute_extraction_diff(
    ...
    if cfg.custom_list_of_wsis:
        ...
        slide_names = [
            f.relative_to(wsi_dir).as_posix()
            for f in wsi_dir.rglob("*")
    ...
    for name in slide_names:
        slide_id = normalize_slide_id(name, root=wsi_dir)
...
def run_pipeline(
    ...
    if cfg.require_complete:
        validation = validate_outputs(cfg, tasks)
        if validation["missing"]:
            raise RuntimeError(
                "Feature extraction is incomplete: "
                f"{len(validation['missing'])}/{validation['total']} expected slides are missing. "
                "Inspect the processor errors or set extraction.require_complete=false "
                "only for an intentional partial-cohort run."
            )
...
def validate_outputs(cfg: TridentExtractionConfig, tasks: list[str]) -> dict:
    ...
    if cfg.custom_list_of_wsis:
        expected = {
            normalize_slide_id(line, root=wsi_dir)
            for line in Path(cfg.custom_list_of_wsis).read_text().splitlines()
            if line.strip()
        }
    elif cfg.search_nested:
        expected = {
            normalize_slide_id(f.relative_to(wsi_dir).as_posix())
            for f in wsi_dir.rglob("*")
            if f.suffix.lower() in extensions
        }
    else:
        expected = {
            normalize_slide_id(f.name) for f in wsi_dir.iterdir() if f.suffix.lower() in extensions
        }
