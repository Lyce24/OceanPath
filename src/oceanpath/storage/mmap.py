"""
Build chunked mmap stores from per-slide H5 feature files.
...
class MmapBuildConfig:
    ...
    require_all_csv_slides: bool = True  # fail if a manifest slide has no valid H5
...
def scan_h5_dir(cfg: MmapBuildConfig) -> tuple[list[SlideInfo], list[ScanError]]:
    ...
    for h5_path in tqdm(h5_paths, desc="Scanning H5s", unit="file"):
        ...
        try:
            ...
            if feat_dim <= 0:
                errors.append(
                    ScanError(slide_id, str(h5_path), f"Invalid feature dimension: {feat_dim}")
                )
                continue
            if coord_dim <= 0:
                errors.append(
                    ScanError(slide_id, str(h5_path), f"Invalid coordinate dimension: {coord_dim}")
                )
                continue
    ...
    if slides:
        ...
        coord_dims = {s.coord_dim for s in slides}
        if len(coord_dims) > 1:
            raise ValueError(
                f"Inconsistent coordinate dimensions across slides: {coord_dims}. "
                "All H5 files must use the same coordinate layout."
            )

    slides.sort(key=lambda s: s.slide_id)

    if allowed_ids is not None and cfg.require_all_csv_slides:
        valid_ids = {slide.slide_id for slide in slides}
        missing_ids = sorted(allowed_ids - valid_ids)
        if missing_ids:
            raise ValueError(
                f"{len(missing_ids)}/{len(allowed_ids)} manifest slide IDs have no valid H5 "
                f"feature file in {h5_dir}. First 10: {missing_ids[:10]}. "
                "Fix feature extraction or set storage.require_all_csv_slides=false "
                "to explicitly allow an incomplete cohort."
            )
...
def _load_allowed_slide_ids(cfg: MmapBuildConfig) -> set[str]:
    ...
    if cfg.csv_id_col in df.columns:
        column = cfg.csv_id_col
    elif cfg.csv_filename_col in df.columns:
        column = cfg.csv_filename_col
    else:
        raise ValueError(
    ...
    if df[column].isna().any():
        ...
        raise ValueError(
            f"CSV {cfg.csv_path} contains missing slide IDs in column '{column}' "
            f"at rows {missing_rows[:10]}"
        )

    slide_ids = {normalize_slide_id(value) for value in df[column].astype(str)}
    if "" in slide_ids:
        raise ValueError(f"CSV {cfg.csv_path} contains an empty slide ID in column '{column}'")
    return slide_ids
    ...
    def write_streamed(
        ...
        if subsample_indices is not None:
            ...
            for batch_start in range(0, len(sorted_idx), chunk_size):
                ...
                block = h5_dataset[0, batch_idx] if has_leading_dim else h5_dataset[batch_idx]
...
def compute_source_hash(slides: list[SlideInfo], cfg: MmapBuildConfig) -> str:
    ...
    for s in slides:
        source_stat = Path(s.h5_path).stat()
        h.update(
            (
                f"slide={s.slide_id};stored={s.n_patches};raw={s.raw_patches};"
                f"feat_dim={s.feat_dim};coord_dim={s.coord_dim};"
                f"size={source_stat.st_size};mtime_ns={source_stat.st_mtime_ns}\n"
            ).encode()
        )

    h.update(f"schema={MMAP_SCHEMA_VERSION}".encode())
    h.update(f"precision={cfg.feat_precision}".encode())
    h.update(f"max_instances={cfg.max_instances}".encode())
    h.update(f"max_chunk_bytes={cfg.max_chunk_bytes}".encode())
    ...
    h.update(f"save_coverage_thumbnails={cfg.save_coverage_thumbnails}".encode())
    if cfg.save_coverage_thumbnails:
        h.update(f"coverage_montage_n={cfg.coverage_montage_n}".encode())
...
def build_mmap(cfg: MmapBuildConfig, force: bool = False) -> BuildResult:
    ...
    if cfg.feat_precision not in {16, 32}:
        raise ValueError(f"feat_precision must be 16 or 32, got {cfg.feat_precision}")
    if cfg.max_chunk_bytes <= 0:
        raise ValueError(f"max_chunk_gb must be positive, got {cfg.max_chunk_gb}")
    if cfg.stream_chunk_size <= 0:
        raise ValueError(f"stream_chunk_size must be positive, got {cfg.stream_chunk_size}")
    if cfg.max_instances is not None and cfg.max_instances <= 0:
        raise ValueError(f"max_instances must be positive or null, got {cfg.max_instances}")
    if cfg.cap_grid_size <= 0:
        raise ValueError(f"cap_grid_size must be positive, got {cfg.cap_grid_size}")
    if cfg.coverage_montage_n < 0:
        raise ValueError(f"coverage_montage_n must be non-negative, got {cfg.coverage_montage_n}")
    try:
        coord_dtype = cfg.coord_np_dtype
    except TypeError as exc:
        raise ValueError(f"Invalid coord_dtype: {cfg.coord_dtype}") from exc
    if coord_dtype.kind not in {"i", "u", "f"}:
        raise ValueError(f"coord_dtype must be numeric, got {cfg.coord_dtype}")

    # Coverage-QC rendering lives in the viz package; the build only produces
    # the grid data (build_coverage_grid) and hands it to the report writer.
    ...
    if not force and existing_hash_path.is_file():
        ...
        if existing_hash == source_hash:
            try:
                existing_meta = validate_mmap_dir(output_dir)
            except (FileNotFoundError, KeyError, OSError, TypeError, ValueError) as exc:
                logger.warning(
                    "Source hash matches, but the existing mmap failed validation (%s); "
                    "rebuilding.",
                    exc,
                )
            else:
                ...
                return BuildResult(
                    ...
                    feat_dim=feat_dim,
                    feat_chunks=int(existing_meta["num_feat_chunks"]),
                    coord_chunks=int(existing_meta["num_coord_chunks"]),
    ...
    # Invalidate the old store before touching its binaries. If this build
    # fails, readers must not see a stale index/source hash paired with
    # partially rewritten chunks.
    for metadata_name in (".source_hash", ".schema_version", "index_arrays.npz", "errors.txt"):
    ...
    for i, slide in enumerate(tqdm(slides, desc="Writing chunks", unit="slide")):
        ...
        try:
            ...
            coord_writer.close()
            raise RuntimeError(
                f"Failed to write slide '{slide.slide_id}'. The mmap build was aborted and "
    ...
    if errors:
        ...
        with open(err_path, "w") as f:
            for error in errors:
                f.write(f"{error.slide_id}\t{error.reason}\n")
        logger.warning(f"{len(errors)} errors logged to {err_path}")

    # ── Finalize QC ──────────────────────────────────────────────────────
    # The build collected the coverage *data* (qc_stats + qc_grids). Rendering
    # lives entirely in oceanpath.viz — storage does not import matplotlib.
    if cfg.save_coverage_thumbnails and qc_stats:
        write_coverage_report(
            ...
            qc_stats=qc_stats,
            qc_grids=qc_grids,
            montage_n=cfg.coverage_montage_n,
        )

    # Free montage grids
    del qc_grids

    # Publish the cache identity only after the complete binary/index store has
    # passed the same structural validation used by downstream datasets.
    validate_mmap_dir(output_dir, require_source_hash=False)
...
def validate_mmap_dir(mmap_dir: str | Path, *, require_source_hash: bool = True) -> dict:
    """
    Validate that a memmap directory contains a valid, complete store.

    Returns metadata dict if valid, raises on failure. Downstream readers
    require the source-identity marker; the builder disables that single check
    while validating immediately before it publishes the marker.
    """
    ...
    if not schema_path.is_file():
        raise FileNotFoundError(f"Schema version marker not found: {schema_path}")
    ...
    source_hash_path = mmap_dir / ".source_hash"
    source_hash = source_hash_path.read_text().strip() if source_hash_path.is_file() else ""
    if require_source_hash and not source_hash:
        raise FileNotFoundError(f"Source hash marker not found: {source_hash_path}")
    if source_hash and (
        len(source_hash) != 20 or any(c not in "0123456789abcdef" for c in source_hash)
    ):
        raise ValueError(f"Invalid source hash marker: {source_hash_path}")
    ...
    with np.load(str(index_path), allow_pickle=True) as idx:
        missing_fields = sorted(required_fields - set(idx.files))
        if missing_fields:
            raise ValueError(f"Mmap index is missing required fields: {missing_fields}")

        index_version = int(idx["schema_version"])
        if index_version != version:
            raise ValueError(
                f"Mmap index schema version {index_version} does not match marker {version}"
            )

        slide_ids = np.asarray(idx["slide_ids"])
        if slide_ids.ndim != 1 or not all(isinstance(s, str) for s in slide_ids):
            raise TypeError(
                "slide_ids in index_arrays.npz must be a one-dimensional string array. "
                "Index may be corrupted — rebuild the mmap."
            )
        if len(set(slide_ids.tolist())) != len(slide_ids):
            raise ValueError("slide_ids in index_arrays.npz must be unique")
        ...
        for name in integer_array_names:
            value = np.asarray(idx[name])
            if value.ndim != 1 or not np.issubdtype(value.dtype, np.integer):
                raise TypeError(
                    f"Mmap index field '{name}' must be a one-dimensional integer array"
                )
            arrays[name] = value.astype(np.int64, copy=False)

        n_slides = int(idx["n_slides"])
        if n_slides <= 0:
            raise ValueError(f"Mmap index must contain at least one slide, got {n_slides}")
        for name, value in {"slide_ids": slide_ids, **arrays}.items():
            if len(value) != n_slides:
                raise ValueError(
                    f"Mmap index field '{name}' has {len(value)} rows, expected {n_slides}"
                )

        lengths = arrays["lengths"]
        if np.any(lengths <= 0):
            bad_rows = np.flatnonzero(lengths <= 0).tolist()
            raise ValueError(f"Mmap lengths must be positive; invalid rows: {bad_rows[:10]}")

        total_patches = int(idx["total_patches"])
        actual_total = int(lengths.sum())
        if total_patches != actual_total:
            raise ValueError(
                f"Mmap total_patches mismatch: index={total_patches}, lengths={actual_total}"
            )

        if "raw_lengths" in idx.files:
            raw_lengths = np.asarray(idx["raw_lengths"])
            if (
                raw_lengths.ndim != 1
                or not np.issubdtype(raw_lengths.dtype, np.integer)
                or len(raw_lengths) != n_slides
            ):
                raise TypeError("Mmap index field 'raw_lengths' must match the slide arrays")
            if np.any(raw_lengths.astype(np.int64, copy=False) < lengths):
                raise ValueError("Mmap raw_lengths cannot be smaller than stored lengths")

        feat_dim = int(idx["feat_dim"])
        coord_dim = int(idx["coord_dim"])
        if feat_dim <= 0 or coord_dim <= 0:
            raise ValueError(
                f"Mmap dimensions must be positive, got feat_dim={feat_dim}, coord_dim={coord_dim}"
            )

        try:
            feat_dtype = np.dtype(str(np.asarray(idx["feat_dtype"]).reshape(-1)[0]))
            coord_dtype = np.dtype(str(np.asarray(idx["coord_dtype"]).reshape(-1)[0]))
        except (IndexError, TypeError) as exc:
            raise ValueError("Mmap index contains an invalid feature or coordinate dtype") from exc
        if feat_dtype.kind != "f":
            raise TypeError(f"Mmap feature dtype must be floating point, got {feat_dtype}")
        if coord_dtype.kind not in {"i", "u", "f"}:
            raise TypeError(f"Mmap coordinate dtype must be numeric, got {coord_dtype}")
        if "bytes_per_feat" in idx.files and int(idx["bytes_per_feat"]) != feat_dtype.itemsize:
            raise ValueError("Mmap bytes_per_feat does not match feat_dtype")

        n_feat_chunks = int(idx["num_feat_chunks"])
        n_coord_chunks = int(idx["num_coord_chunks"])
        if n_feat_chunks <= 0 or n_coord_chunks <= 0:
            raise ValueError(
                "Mmap chunk counts must be positive, got "
                f"features={n_feat_chunks}, coords={n_coord_chunks}"
            )
        ...
        def validate_chunks(
            *,
            ...
            if np.any((chunk_ids < 0) | (chunk_ids >= chunk_count)):
                bad_rows = np.flatnonzero((chunk_ids < 0) | (chunk_ids >= chunk_count)).tolist()
                raise ValueError(f"Invalid {prefix} chunk IDs at rows: {bad_rows[:10]}")
            if np.any(offsets < 0):
                bad_rows = np.flatnonzero(offsets < 0).tolist()
                raise ValueError(f"Negative {prefix} offsets at rows: {bad_rows[:10]}")
            if np.any(offsets % dtype.itemsize != 0):
                bad_rows = np.flatnonzero(offsets % dtype.itemsize != 0).tolist()
                raise ValueError(f"Unaligned {prefix} offsets at rows: {bad_rows[:10]}")

            expected_paths = {mmap_dir / f"{prefix}_{i:03d}.bin" for i in range(chunk_count)}
            for path in expected_paths:
                if not path.is_file():
                    raise FileNotFoundError(f"Missing {prefix} chunk: {path}")
            actual_paths = set(mmap_dir.glob(f"{prefix}_*.bin"))
            extra_paths = sorted(str(path) for path in actual_paths - expected_paths)
            if extra_paths:
                raise ValueError(f"Unindexed {prefix} chunks found: {extra_paths}")

            bytes_per_row = row_width * dtype.itemsize
            for chunk_id in range(chunk_count):
                path = mmap_dir / f"{prefix}_{chunk_id:03d}.bin"
                file_size = path.stat().st_size
                if file_size % dtype.itemsize != 0:
                    raise ValueError(
                        f"{prefix} chunk {path.name} has a byte size not aligned to {dtype}"
                    )
                ...
                spans = sorted(
                    (
                        int(offsets[row]),
                        int(offsets[row]) + int(lengths[row]) * bytes_per_row,
                        int(row),
                    )
                    for row in rows
                )
                ...
                for start, end, row in spans:
                    if start != expected_offset:
                        raise ValueError(
                            f"Invalid {prefix} offset for slide '{slide_ids[row]}': "
                            f"expected {expected_offset}, got {start}"
                        )
                    if end > file_size:
                        raise ValueError(
                            f"Truncated {prefix} chunk {path.name}: slide '{slide_ids[row]}' "
                            f"requires byte {end}, file has {file_size} bytes"
                        )
                    expected_offset = end

                if expected_offset != file_size:
                    raise ValueError(
                        f"{prefix} chunk {path.name} size mismatch: index accounts for "
                        f"{expected_offset} bytes, file has {file_size}"
                    )

        validate_chunks(
            prefix="features",
            chunk_count=n_feat_chunks,
            chunk_ids=arrays["feat_chunk_ids"],
            offsets=arrays["feat_offsets"],
            row_width=feat_dim,
            dtype=feat_dtype,
        )
        validate_chunks(
            prefix="coords",
            chunk_count=n_coord_chunks,
            chunk_ids=arrays["coord_chunk_ids"],
            offsets=arrays["coord_offsets"],
            row_width=coord_dim,
            dtype=coord_dtype,
        )
