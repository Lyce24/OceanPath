"""
Stage 5: Basic model evaluation.

Reads Stage 4 training outputs and produces:
  ...
  Part 2 — Held-out final model evaluation (best_fold, ensemble, refit):
  ...
  Held-out results are reporting-only. They never choose the export strategy.
...
Usage:
    ...
    python scripts/evaluate.py platform=local data=blca encoder=univ1 \\
...
from oceanpath.runtime import Reporter
...
#
# This workflow decides WHAT to report; the reporter (built via
# oceanpath.runtime.build_reporter) owns HOW it reaches the tracker. No wandb
# import lives here — swap the tracker in oceanpath.runtime.reporting.
# ═════════════════════════════════════════════════════════════════════════════


def _report_oof(reporter: Reporter, oof_result: dict) -> None:
    """Report OOF evaluation metrics."""
    summary: dict = {}
    ...
    if oof_result.get("n_slides"):
        summary["oof/n_slides"] = oof_result["n_slides"]
    if oof_result.get("n_patients"):
        summary["oof/n_patients"] = oof_result["n_patients"]

    reporter.log_summary(summary)


def _report_final_model(reporter: Reporter, model_name: str, result: dict) -> None:
    """Report a single final model's evaluation results."""
    if "error" in result:
        return
        ...
        for metric_name, metric_data in level_data.items():
            if isinstance(metric_data, dict):
                ...
                if point is not None and np.isfinite(point):
                    summary[f"final/{model_name}/{level_short}/{metric_name}"] = point
                ci_lo = metric_data.get("ci_lower")
                ci_hi = metric_data.get("ci_upper")
                if ci_lo is not None:
                    summary[f"final/{model_name}/{level_short}/{metric_name}_ci_lower"] = ci_lo
                    summary[f"final/{model_name}/{level_short}/{metric_name}_ci_upper"] = ci_hi

    # Calibration
    cal = result.get("calibration", {})
    ece = cal.get("ece")
    if ece is not None and isinstance(ece, (int, float)) and np.isfinite(ece):
        summary[f"final/{model_name}/ece"] = ece
    brier = cal.get("brier")
    if brier is not None and isinstance(brier, (int, float)) and np.isfinite(brier):
        summary[f"final/{model_name}/brier"] = brier

    # Stability
    stab = result.get("threshold_stability", {})
    if stab.get("stability_grade"):
        summary[f"final/{model_name}/stability_grade"] = stab["stability_grade"]

    reporter.log_summary(summary)


def _report_plots(reporter: Reporter, eval_dir: Path) -> None:
    """Upload evaluation plots as image artifacts."""
    ...
    for model_dir in final_models_dir.iterdir():
        ...
        for png in sorted(plots_dir.glob("*.png")):
            reporter.log_image(f"plots/{model_name}/{png.stem}", png)
...
def evaluate_oof(
    ...
    if not oof_path.is_file():
        raise FileNotFoundError(
            f"Required OOF predictions not found: {oof_path}. "
...
def evaluate_final_models(
    ...
    """
    Run inference and evaluate final models on an independent held-out cohort.

    With no held-out test IDs the valid scope is OOF-only, so final-model
    ranking is skipped. If skip_inference=True, existing held-out predictions
    are required in each final model's evaluation directory.
    """
    from oceanpath.config import FoundationPaths
    ...
    if not test_ids:
        reason = (
            "No held-out test cohort is defined by the split profile. "
            "Final-model held-out inference was skipped; the evaluation is OOF-only."
        )
        ...
        _save_json(
            ...
            eval_dir / "evaluation_scope.json",
        )
        _remove_stale_comparison_outputs(eval_dir)
        return {}
    ...
    model_results = {}
    configured_strategies = OmegaConf.select(
        cfg,
        "eval.final_model_strategies",
        default=["best_fold"],
    )
    strategies = [str(value) for value in configured_strategies]
    supported = {"best_fold", "ensemble", "refit"}
    invalid = sorted(set(strategies) - supported)
    if not strategies or invalid:
        raise ValueError(
            "eval.final_model_strategies must be a non-empty list drawn from "
...
def _attach_patient_id(
    ...
    try:
        ...
        if patient_column in csv_df.columns and filename_column in csv_df.columns:
            csv_df["_slide_id"] = csv_df[filename_column].map(normalize_slide_id)
...
def _remove_legacy_selection_outputs(eval_dir: Path) -> None:
    """Remove artifacts from the retired test-set model-selection behavior."""
    stale_paths = (
        eval_dir / "model_comparison.json",
        eval_dir / "recommended_model.json",
...
def _remove_stale_comparison_outputs(eval_dir: Path) -> None:
    """Remove legacy selection outputs and held-out outputs from an OOF-only run."""
    _remove_legacy_selection_outputs(eval_dir)
    final_models_dir = eval_dir / "final_models"
    if final_models_dir.is_dir():
...
def _run_evaluation(cfg: DictConfig, paths: FoundationPaths) -> None:
    """
    Stage 5 entry point.
    ...
    if cfg.get("dry_run", False):
        print(f"\n{'=' * 60}")
        print("  DRY RUN — evaluate.py")
        print(f"{'=' * 60}")
        print(f"  Training input: {train_dir}")
        print(f"  Output:         {eval_dir}")
        print(f"  N folds:        {_get_n_folds(cfg)}")
        print(f"{'=' * 60}\n")
        return
    ...
    # ── Experiment reporter ──────────────────────────────────────────────
    from oceanpath.runtime import build_reporter
    ...
    _report_oof(reporter, oof_result)
    ...
    for name, result in model_results.items():
        _report_final_model(reporter, name, result)

    if not valid_results:
        logger.warning("No valid final models were evaluated")

    # Upload plots to W&B
    _report_plots(reporter, eval_dir)
...
def run_evaluation(cfg: DictConfig) -> StageResult:
    """Run inference and basic evaluation for a training experiment."""
