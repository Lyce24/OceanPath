"""
Optional throughput benchmarking with bottleneck identification.
...
Run after any architecture or data pipeline change:
    python scripts/benchmark.py platform=local data=blca encoder=univ1 \\
           model=abmil training=default
...
def benchmark_data_loading(
    ...
    from oceanpath.datasets import MmapDataset
    ...
    try:
        ...
        label_map = {
            normalize_slide_id(row[filename_col]): int(row[label_col])
...
def _build_model(cfg: DictConfig, device: str):
    ...
    # synthetic benchmarking. 4000 is representative of a medium WSI bag.
...
def run_benchmark(cfg: DictConfig) -> None:
    """Run benchmark phases with standard provenance and status metadata."""
    ...
    with run_context(
        ...
        persist=not bool(cfg.get("dry_run", False)),
    ):
        _run_benchmark(cfg)
