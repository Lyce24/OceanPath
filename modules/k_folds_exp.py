...
class ExperimentLIT:
    ...
    def _train_fold(self, fold: int) -> Dict[str, Any]:
        ...
        # Optional per-fold threshold tuning (binary only)
        if self.config.use_optimal_threshold and y_prob.shape[1] == 2:
            p_pos = y_prob[:, 1]
            fold_threshold, j_score = tune_threshold_youden(y_true, p_pos)
            op_metrics = operating_point_metrics(y_true, p_pos, fold_threshold)
        else:
            j_score = None

        fold_metrics = compute_metrics(y_true, y_prob, threshold=fold_threshold if y_prob.shape[1] == 2 else None)
        result = {
            "fold": fold,
            "val_ids": list(dm.val_ids),

            # Fold predictions (kept only if you still want them; optional)
            "logits": logits,
            "labels": labels,

            # Training selection info
            ...
            # Regular K-fold: fold-level evaluation
            "fold_metrics": fold_metrics,
            "fold_threshold": float(fold_threshold),
            "fold_op_metrics": op_metrics,
            "fold_youden_j": float(j_score) if j_score is not None else None,
        }

        for k, v in fold_metrics.items():
            self._log_summary(f"fold_{fold}/val_{k}", v)
        self._log_summary(f"fold_{fold}/threshold", fold_threshold)
        for k, v in op_metrics.items():
            self._log_summary(f"fold_{fold}/{k}", v)
        ...
        if tracker.best_score is not None:
            ...
            self._log_summary(
                f"fold_{fold}/best_epoch",
                (tracker.best_epoch + 1) if tracker.best_epoch is not None else None
            )
    ...
    def run_cv(self) -> Dict[str, Any]:
        ...
        print(f"\n{'='*50}")
        print(f"Running {len(folds)}-fold CV (REGULAR K-FOLD)")
        ...
        # (Only aggregate keys that exist in all folds and are finite.)
        metric_keys = set()
        for r in self._fold_results:
            metric_keys |= set((r.get("fold_metrics") or {}).keys())

        agg: Dict[str, Dict[str, float]] = {}
        for mk in sorted(metric_keys):
            vals = []
            for r in self._fold_results:
                fm = r.get("fold_metrics") or {}
                v = fm.get(mk, np.nan)
                if np.isfinite(v):
                    vals.append(float(v))
            if vals:
        ...
        # Aggregate thresholds (binary only, if tuned)
        thresholds = [r.get("fold_threshold", 0.5) for r in self._fold_results if r.get("fold_threshold") is not None]
        cv_threshold = float(np.median(thresholds)) if thresholds else 0.5

        print(f"\n{'='*20} CV Summary (mean±std) {'='*20}")
        print(f"Mode: {self.config.mode.upper()}")
        if self.config.use_optimal_threshold:
            print(f"Threshold (median across folds): {cv_threshold:.4f}")

        for k, s in agg.items():
            print(f"  {k}: {s['mean']:.4f} ± {s['std']:.4f} (n={s['n']})")
            self._log_summary(f"cv/{k}", f"{s['mean']:.4f} ± {s['std']:.4f}")
            self._log_summary(f"cv/{k}_mean", s["mean"])
            self._log_summary(f"cv/{k}_std", s["std"])

        self._log_summary("cv/threshold", cv_threshold)
        ...
        for r in self._fold_results:
            ...
            fold_table.append([
                r["fold"],
                r.get("best_score", np.nan),
                (r.get("best_epoch", -1) + 1) if r.get("best_epoch") is not None else None,
                r.get("fold_threshold", 0.5),
                fm.get("roc_auc", np.nan),
                fm.get("f1", np.nan),
                fm.get("accuracy", np.nan),
            ])

        self._log_table(
            "fold_results",
            columns=["fold", "best_score", "best_epoch", "threshold", "roc_auc", "f1", "accuracy"],
            data=fold_table,
        )
    ...
    def run(self, save_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.config.final_strategy == "refit_full":
            ...
            # use average of best epochs
            final_epochs = int(np.ceil(np.mean(epochs))) if epochs else self.config.max_epochs
